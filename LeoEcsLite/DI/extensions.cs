            ...
            for (var i = 0; i < systemsCount; i++) {
                ...
                foreach (var f in system.GetType ().GetFields (BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)) {
                    ...
                    if (system is EcsGroupSystem && f.FieldType == typeof(IEcsSystem[]))
                    {
                        ...
                        foreach (var nestedSystem in nestedSystems)
                        {
                            foreach (var f1 in nestedSystem.GetType().GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
                            {
                                // skip statics.
                                if (f1.IsStatic) { continue; }
                                // EcsWorldInject, EcsFilterInject, EcsPoolInject, EcsSharedInject.
                                if (InjectBuiltIns(f1, nestedSystem, systems)) { continue; }
                                // EcsDataInject.
                                if (InjectCustoms(f1, nestedSystem, injects)) { continue; }
                            }
                        }
                    }
